...
with st.container(border = True, height= 300):
    df_exibição = df_filter.drop(['Trânsito_bool', 'Acessibilidade_bool',
       ...
       'Coleta de lixo_bool', 'Calçamento/Pavimentação_bool'], axis= 1)
    st.subheader(f" Registros coletados do PCA no(s) bairro(s)")
    st.dataframe(df_exibição)

list_mensagem = df['Bairro'].unique().tolist()
#MENSAGEM INTERATIVA
mensagem = " - ".join([f"""{i}""" \
                        for i in list_mensagem [:-1]])
...
<div style="
    ...
    <marquee behavior="scroll" direction="left" scrollamount="5" style="color: red; font-size: 20px;">
        🚀🗺️ Bairros Coletados : {} 🌍📊 !!
    </marquee>
</div>
""".format(mensagem)
...
with aba1:
    ...
    with col_1: 
        with st.container(border = True, height= 350):
            ...
            fig_bairro = px.bar(df_exibição, x = 'Bairro', template= 'simple_white', color = 'Gênero',title = 'Respostas Coletadas por Bairro' )
            fig_bairro.update_xaxes(title_text =  ' ')
            fig_bairro.update_yaxes(title_text =  'Respostas Coletadas')
            fig_bairro.update_layout(width = 400, height = 350)
...
with aba3:
    with st.container(border= True,height= 600):
        #tratar os dados de coords para o formato certo
        # trocar o separador decimal

        df_filter['lat'] = df_filter['lat'].str.replace(',','.')
        df_filter['lon'] = df_filter['lon'].str.replace(',', '.')
        ...
        #passando para float 64 para evitar o erro no deploy
